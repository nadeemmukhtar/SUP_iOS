...
//  sup
//
//  Created by Robert Malko on 6/25/20.
...
extension UIApplication {
    func currentDeviceOrientation(cameraPosition pos: AVCaptureDevice.Position) -> OTVideoOrientation {
        guard let orientation = UIApplication.topMostScene()?.interfaceOrientation else {
            return .left
        }
...
class CustomVideoCapture: NSObject, OTVideoCapture {
    ...
    fileprivate func updateFrameOrientation() {
        DispatchQueue.main.async {
            ...
            self.videoFrameOrientation =
                UIApplication.shared.currentDeviceOrientation(
                    cameraPosition: inputDevice.device.position
            )
        }
    }
    ...
    override init() {
        ...
        (captureWidth, captureHeight) = capturePreset.dimensionForCapturePreset()
        videoFrame = OTVideoFrame(
            format: OTVideoFormat(nv12WithWidth: captureWidth, height: captureHeight)
        )

        AppPublishers.backgroundStatusChanged$.sink { status in
    ...
    fileprivate func setupAudioVideoSession() throws {
        // print("zz CustomVideoCapture setupAudioVideoSession")
        ...
        // print("zz CustomVideoCapture setupAudioVideoSession - startRunning")
    }
    ...
    func initCapture() {
        // print("zz CustomVideoCapture initCapture")
    ...
    func start() -> Int32 {
        // print("zz CustomVideoCapture start")
    ...
    func stop() -> Int32 {
        // print("zz CustomVideoCapture stop")
        capturing = false
        return 0
    }

    func releaseCapture() {
        // print("zz CustomVideoCapture releaseCapture")
        let _ = stop()
    ...
    fileprivate var hasMultipleCameras : Bool {
        // AVCaptureDevice.devices(for: AVMediaType.video).count > 1

        return AVCaptureDevice.DiscoverySession.init(
...
extension CustomVideoCapture: FrameCapturerMetadataDelegate {
    func finishPreparingFrame(_ videoFrame: OTVideoFrame?) {}
}

extension CustomVideoCapture: AVCaptureVideoDataOutputSampleBufferDelegate {
...
class CustomVideoRenderer: UIView, OTVideoRender {
